...
class Banners extends Model {
    ...
    public function modiBanner($id, $bname = '', $bimg = '', $bposition = 0, $reltype = 0, $relid = false, $sort = 0, $href = '#', $exp = '') {
        ...
        if ($id > 0) {
            ...
            // 删除
            $id = abs($id);
            return $this->Dao->delete()->from(TABLE_BANNERS)->where("id=$id")->exec() > 0;
        } else if ($id == 0 || !$id) {
            // 添加
            return $this->Dao->insert(TABLE_BANNERS, 'banner_name,banner_href,banner_image,banner_position,reltype,relid,sort,exp')
                            ->values(array($bname, $href, $bimg, $bposition, $reltype, $relid, $sort, $exp))->exec();
        } else {
            return false;
        }
    }
    ...
    public function getOne($id) {
        return $this->Dao->select()->from(TABLE_BANNERS)->where("id=$id")->getOneRow();
    }
