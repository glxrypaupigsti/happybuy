...
class WechatSdk extends Model {
    ...
    public static function getServiceAccessToken() {
        ...
        $data = json_decode(file_get_contents(dirname(__FILE__) . "/json_cache/access_token.json"));
        if ($data->expire_time < time()) {
            ...
            if ($access_token) {
                ...
                $fp = fopen(dirname(__FILE__) . "/json_cache/access_token.json", "w");
    ...
    public static function getAccessToken($code) {
        ...
        $_return->expires = $Result['expires_in'];
        return $_return;
    }
    ...
    public static function deleteMenu(){
    	$access_token = self::getServiceAccessToken();
    	$jsonStr = str_replace('\"', '"', $jsonStr);
    	return json_decode(Curl::post("https://api.weixin.qq.com/cgi-bin/menu/delete?access_token=$access_token", $jsonStr), true);
    }
    ...
    public static function getUserGroup($update = false) {
        ...
        $data = json_decode(file_get_contents(dirname(__FILE__) . "/json_cache/user_group.json"), true);
        if ($data['expire_time'] < time() || $update) {
            ...
            if (!isset($res['errcode'])) {
                ...
                $fp = fopen(dirname(__FILE__) . "/json_cache/user_group.json", "w");
                fwrite($fp, json_encode($data));
                fclose($fp);
                return $data['groups'];
            }
     ...
     * @return type
     */
    public static function getUserSummary($end_date, $dataOffset = 6) {
        $stoken = self::getServiceAccessToken();
        $begin_date = date("Y-m-d", strtotime("$end_date -$dataOffset day"));
        $PostData = array(
...
class Messager {
    ...
    public static function sendNotification($access_token, $openid, $content, $url)
    {
        $RequestUrl = "https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token=$access_token";
        $postData = array();
        $postData['touser'] = (string) $openid;
