    ...
    public function alterGmess($msgId, $title, $content, $desc, $catImg, $thumbMediaId = null, $category = 0) {
    	...
    	if ($msgId > 0) {
            ...
            return $this->Dao->update(TABLE_GMESS)
                            ->set($data)->where('id', $msgId)->exec();
        } else {
            // 插入数据
        	$time =  date("Y-m-d",time());
            return $this->Dao->insert(TABLE_GMESS, 'title, content, `desc`, catimg, category, thumb_media_id,createtime')
                            ->values(array($title, $content, $desc, $catImg, $category, $thumbMediaId,$time))->exec();
        }
    }
