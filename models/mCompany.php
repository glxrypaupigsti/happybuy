...
class mCompany extends Model {
    ...
    public function getCompanys($verifed = 1) {
        return $this->Db->query("SELECT * FROM companys WHERE verifed = $verifed AND `deleted` = 0 ORDER BY id DESC;");
    }
    ...
    public function getCompanysPairs($verifed = 1) {
        return $this->Db->query("SELECT `id`,`name` FROM companys WHERE verifed = $verifed AND `deleted` = 0 ORDER BY id DESC;");
    }
    ...
    public function getCompanyInfo($id) {
        if (is_numeric($id)) {
            return $this->Db->getOneRow("SELECT * FROM companys WHERE id = $id;");
        } else {
            return false;
        }
    }
