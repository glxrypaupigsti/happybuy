...
<div style="margin-bottom: 60px;padding: 10px 20px;">
        ...
        <div class="fv2Right" style="margin-left: 0;">
            <div class="clearfix">
                {foreach from=$exps item=exp key=k}
                    <a class="expitem exps" data-k="{$k}" href="javascript:;">{$exp}</a>
                {/foreach}
            </div>
        </div>
    </div>
    ...
    <input type="hidden" id="exp_couriers" value="{$settings.exp_couriers}" />

    <p class="Thead">设置配送处理人员</p>
    <div class="fv2Field clearfix" style="max-width: 100%;">
        <div class="fv2Right" style="margin-left: 0;">
            <div class="clearfix">
                {foreach from=$couries item=courier key=k}
                    <a class="expitem couriers" data-k="{$k}" href="javascript:;">{$courier}</a>
                {/foreach}
            </div>
        </div>
    </div>
