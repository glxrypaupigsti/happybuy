...
<html>
    ...
    <body>
        ...
        <div id="product_list" class="clearfix" style="margin-top: -1px;">        
            <div class="clearfix pdBlock">
                {foreach from=$pds item=pd}
                    <section class="productListWrap hoz" onclick="location = '{$docroot}?/vProduct/view/id={$pd.product_id}&showwxpaytitle=1';">
                        <a class="productList">
                            <img class="photo" src="{if $config.usecdn}{$config.imagesPrefix}product_hpic/{$pd.catimg}_x200{else}static/Thumbnail/?w=200&h=200&p={$config.productPicLink}{$pd.catimg}{/if}" />           
                            <section>
            ...
            <script data-main="{$docroot}static/script/Wshop/shop_vbrand.js?v={$cssversion}" src="{$docroot}static/script/require.min.js"></script>
