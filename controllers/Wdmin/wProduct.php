...
class wProduct extends Controller {
    ...
    public function ajax_product_list() {
        ...
        $key = $this->pPost('key', '');
        if ($product_cat > 0) {
        	$search_key_condition = !empty($key) ? " and `product_name` LIKE '%$key%'" : '';
        	$product_cat_condition = empty($key) ? " and `product_cat` = ".$product_cat : '';
        	
        	$SQL = 'select po.catimg,po.product_id,po.product_name,po.product_online,po.product_code,po.product_readi,ps.sale_prices,psl.serial_name,pca.cat_parent'
        			  .' from ' .TABLE_PRODUCTS . ' as po' 
        			  .' left join ' . TABLE_PRODUCT_ONSALE .' as ps on ps.product_id=po.product_id'
        			  .' left join ' . TABLE_PRODUCT_SERIALS . ' as psl on psl.id = po.product_serial'
        			  .' left join ' . TABLE_PRODUCT_CATEGORY . ' as pca on pca.cat_id = po.product_cat'
        			  .' where po.`delete` <> 1 ' . $search_key_condition . $product_cat_condition;
        	$countSql = "select count(1) as count from (".$SQL.') c';
        	$SQL = $SQL . " order by po.product_id DESC limit $offset,$page_size";
        	
        	$pds = $this->Db->query($SQL);
        	$countArr = $this->Db->query($countSql);
        	$count = intval($countArr[0]['count']);
        	$totalPage = $count / $page_size;
        	
        	error_log("query order list sql ======>" .$SQL);
        	error_log("query order list countArr ======>" .json_encode($countArr));
        	error_log("pds list count ======>" .json_encode($pds));
        	error_log("total page count ======>" .$totalPage);
        	
//             $pds = $this->Dao->select("po.catimg,po.product_id,po.product_name,po.product_online,po.product_code,po.product_readi,ps.sale_prices,psl.serial_name,pca.cat_parent")
//                     ->from(TABLE_PRODUCTS)->alias('po')
...
//                 'count' => sizeof($pds)
    ...
    public function switchOnline() {
        ...
        $this->Carts->del_cart_product_by_product_id($productId);
...
//         $cats[] = array('name' => '未分类', 'dataId' => 0, 'hasChildren' => false, 'children' => array(), 'open' => 'true');
    ...
    public function productReverse() {
        if (is_numeric($this->post('pid'))) {
            ...
            if ($pid > 0) {
                echo $this->Db->query("UPDATE `products_info` SET `delete` = 0 WHERE product_id = $pid;");
            } else if ($pid == 0) {
                // 全部还原
                echo $this->Db->query("UPDATE `products_info` SET `delete` = 0 WHERE `delete` = 1;");
            }
        } else {
            echo 0;
        }
    ...
    public function removeProduct() {
        if (is_numeric($this->post('pid'))) {
            ...
            $this->loadModel('Product');
            if ($pid == 0) {
                $products = $this->Dao->select('product_id')->from(TABLE_PRODUCTS)->where('`delete` = 1')->exec();
