    ...
    require(['util', 'jquery', 'Spinner', 'Cart'], function (util, $, Spinner, Cart) {
        ...
        o.loadCartData = function () {
            ...
            if (o.cartData === undefined || o.cartData === null || o.cartData === '{}') {
                ...
                // [HttpPost]
                $.post(shoproot + '?/vProduct/orderData/', {
            ...
            $('#cart-balance-check').click(function () {
            	 var hasChk = $('#cart-balance-check').is(':checked');
                 if (parseFloat($('#cart-balance-pay').text()) > 0 && parseFloat($('#cart-balance-pay').text()) > countOrderAmount() ) {
                    
                

                    if(hasChk){
                         $('#money-payment-btn').show();
                         $('#wechat-payment-btn').hide();
                    }else{
                         
                         $('#wechat-payment-btn').show();
                         $('#money-payment-btn').hide();
        ...
        function wepayCall() {
            ...
            function orderGenhandle(Id) {
                ...
                Cart.clear();
                if ( $('#cart-balance-check')[0].checked) {
                    // 如果订单总额为0.不需要支付
                   // Cart.clear();
                   // window.payed = true;
                   ...
                   // $('#wechat-payment-btn').removeClass('disable').html('微信安全支付');
                	 ...
                	 if (orderId > 0){
                		 ...
                		 $.post(shoproot + "?/Order/ajaxMoneyPay/", {
                             orderId: orderId
                         }, function (result) {
        ...
        $('#money-payment-btn').click(wepayCall);
