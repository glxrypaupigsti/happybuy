...
requirejs(['util', 'fancyBox', 'datatables', 'Spinner', 'jUploader', 'ztree', 'ztree_loader', 'baiduTemplate'], function (util, fancyBox, dataTables, Spinner, jUploader, ztree, treeLoader, baiduTemplate) {
    ...
    var exp_couriers = $('#exp_couriers').val();
    ...
    if(exp_couriers !== ''){
    	$('.couriers').each(function(){
    		 if (exp_couriers.indexOf($(this).attr('data-k')) !== -1) {
                 $(this).addClass('hov');
             }
    	});
    }
    ...
    $('#saveBtn').click(function () {
        hov = [];
        $('.exps.hov').each(function () {
            hov.push($(this).attr('data-k'));
        });
        couries = [];
        $('.couriers.hov').each(function () {
        	couries.push($(this).attr('data-k'));
        });
        var exps = [];
