...
requirejs(['jquery', 'util', 'fancyBox', 'datatables', 'Spinner', 'jUploader', 'ztree', 'ztree_loader'], function ($, util, fancyBox, dataTables, Spinner, jUploader, ztree, treeLoader) {
    ...
    $('#saveBtn').click(function () {
    	...
    	if(null == $('#req').val() || '' == $('#req').val() || isNaN($('#req').val() ) || parseFloat($('#req').val()) <= 0){
    		util.Alert("满额必须为大于0的数字");
    		return;
    	}
    	
    	if(null == $('#dis').val() || '' == $('#dis').val() || isNaN($('#dis').val()) || parseFloat($('#dis').val()) <= 0 ){
    		util.Alert("减额必须为大于0的数字");
    		return;
    	}
