...
requirejs(['jquery', 'util', 'fancyBox', 'datatables', 'Spinner'], function ($, util, fancyBox, dataTables, Spinner) {
     ...
     * 加载菜单数据
     */
    $('#menuX').load(shoproot + '?/WdminAjax/ajaxGetWechatMenu/', function () {
        Spinner.stop();
        fnBtnListen();
        $('.menutop').eq(0).click();
    });
    ...
    $('#menu_pubbtn').on('click', function () {
        var Json = convToJson();
        $('#menu_pubbtn').html('正在发布中');
        $.post(shoproot + '?/WdminAjax/ajaxSetWechatMenu/', {
    ...
    $('#menu_delete').on('click', function () {
        var Json = convToJson();
        $('#menu_delete').html('正在发布中');
        if(confirm('确定要清空微信菜单吗？')){
        	$.post(shoproot + '?/WdminAjax/ajaxClearWechatMenu/', {}, function (R) {
                if (parseInt(R.errcode) === 0) {
                    util.Alert('清空成功！');
                } else {
                    util.Alert('清空失败,系统错误！', true);
                }
                $('#menu_delete').html('清空菜单');
            });
        }
    });
