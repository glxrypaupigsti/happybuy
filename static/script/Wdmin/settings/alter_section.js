...
requirejs(['jquery', 'util', 'fancyBox', 'datatables', 'Spinner', 'jUploader', 'ztree', 'ztree_loader','WdatePicker'], function ($, util, fancyBox, dataTables, Spinner, jUploader, ztree, treeLoader,WdatePicker) {
    ...
    $('#saveBtn').click(function () {
    	var name = $('#name').val();
    	if(null == name || '' == name){
    		util.Alert("板块名称不能为空",true);
    		return;
    	}
    	
    	var banner = $('#banner').val();
    	if(null == banner || '' == banner){
    		util.Alert("图片不能为空",true);
    		return;
    	}
    	...
    	if(null != startTime && '' != startTime){
    		if(null == endTime || '' == endTime){
        		util.Alert("开放截止时间不能为空",true);
        		return;
    		}
    		//比较时间
    		var result = util.timeCompare(startTime,endTime);
    		if(result == 1){
    			util.Alert("开放开始时间不能大于开放截止时间",true);
    			return;
    		}else if(result == 2 ){
    			util.Alert("开放截止时间小于当前时间",true);
    			return;
    		}
    	}
