...
class Controller {
    ...
    final public function getOpenId($redirect_uri = false, $both = false) {
        ...
        if (!$config->wechatVerifyed) {
            ...
            if ((strlen($Openid) > 0) AND (strlen($AccessToken) > 0)) {
                error_log('openid:'.$Openid);
                error_log('AccessToken:'.$AccessToken);
                //$this->refreshOpenId($Openid, $AccessToken);
            } else {
                if ($this->inWechat()) {
                    error_log('request base access code: GET:'.json_encode($_GET));
                    if (!$redirect_uri) {
                        $redirect_uri = $this->uri;
                    }
                    
                    $redirect_uri = preg_replace('/code=[0-9a-zA-Z]+\&/', '', $redirect_uri);
                    $url = $config->domain.'?/iWechat/request_base_token/ret_url='.urlencode($redirect_uri);
                    header("location:" . $url);
                    exit(0);
                    ...
                    if ($AccessCode !== FALSE) {
                        ...
                        error_log('request $AccessCode from weixin failed');
                    }
                    unset($AccessCode);
                     */
                } else {
    ...
    public function initSettings($recache = false) {
        ...
        $ass = $fileCached->get('initSettings', 360);
        if (-1 === $ass || $recache) {
            $ass = array();
            $ret = $this->Dao->select()->from('wshop_settings')->exec();
            foreach ($ret as $r) {
                $ass[$r['key']] = $r['value'];
            }
            $fileCached->set('initSettings', $ass);
        }
        $this->settings = $ass;
     ...
     * @param type $type 默认错误级别
     */
    public function print_log($msg,$msg_header='log'){
    	if(DEBUG){
    		error_log($msg_header."================>".$msg);
    	}
    }
    ...
    /**
     * 输出json消息
     * @param type $error_code 错误码
     * @param type $msg  提示信息
     */
    final public function echoApiMsg($error_code, $msg = '',$data) {
        $this->echoJson(array(
